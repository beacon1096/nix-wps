...
stdenv.mkDerivation rec {
  ...
  installPhase = ''
    ...
    # Use batched sed instead of iterative substituteInPlace loops for performance
    sed -i -e "s|/opt/kingsoft/wps-office|$prefix|g" "$out"/bin/{wps,wpp,et,wpspdf}
    sed -i -e "s|/usr/bin|$out/bin|g" "$out"/share/applications/*.desktop
  ...
  meta = {
    description = "Office suite, formerly Kingsoft Office (XA overseas version)";
